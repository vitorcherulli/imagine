...
export async function POST(req: NextRequest) {
  ...
  if (files.length > 6) {
    return NextResponse.json({ error: "Max 6 images per avatar" }, { status: 400 });
  }
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const url = await saveAvatarBuffer(userId, id, `ref_${i}.${ext}`, buf);
    urls.push(url);
  }
  ...
  return NextResponse.json({ avatar: created });
}
